...
async fn setup_rootshell(
    ...
    #[cfg(feature = "vendor")]
    let rootshell_bin = include_bytes!("../../rootshell");

    #[cfg(not(feature = "vendor"))]
    let rootshell_bin =
        &tokio::fs::read("target/armv7-unknown-linux-gnueabihf/release/rootshell").await?;
...
async fn setup_rayhunter(
    ...
    loop {
        match adb_echo_test(adb_device).await {
            Ok(dev) => {
                adb_device = dev;
